...
query_body =  "&utf8=%E2%9C%93&work_search%5Bquery%5D=&work_search%5Btitle%5D=&work_search%5Bcreators%5D=&work_search%5Brevised_at%5D=&work_search%5Bcomplete%5D=0&work_search%5Bsingle_chapter%5D=0&work_search%5Bword_count%5D=%3E5000&work_search%5Blanguage_id%5D=1&work_search%5Bfandom_names%5D=Lord+of+the+Flies&work_search%5Brating_ids%5D=&work_search%5Bcharacter_names%5D=&work_search%5Brelationship_names%5D=&work_search%5Bfreeform_names%5D=&work_search%5Bhits%5D=&work_search%5Bkudos_count%5D=&work_search%5Bcomments_count%5D=&work_search%5Bbookmarks_count%5D=&work_search%5Bsort_column%5D=&work_search%5Bsort_direction%5D=&commit=Search"
pages = 3
...
def get_ids(query_base, query_body, pages): 
    """
...
def get_text(htmlfolder, txtfolder): 
    ...
    Save the text content to a plain text file in a separate folder. 
    """
...
def get_tags(htmlfolder): 
    ...
    First, builds lists of tags and metadata for each work.
    Second, transforms the lists to a work x tags matrix.
    """
    # Lists
    all_tags = []
    all_titles = []
    ...
    for file in glob.glob("html/*.html"): 
        ...
        with open(file, "r", encoding="utf8") as infile: 
            content = infile.read()
            textlen = re.findall("<dd class=\"words\">(.*?)</dd>", content)
            print(textlen[0])
    ...
    # Filter for number of times a tag occurs in the collection. (Many tags appear very rarely.)
    min_count = 15 ## adjust minimal number a tag must appear to be saved.
    all_tags.drop([col for col, val in all_tags.sum().iteritems() if val < min_count], axis=1, inplace=True)
...
def main(wdir, a3o_base, query_base, query_body, pages, htmlfolder, txtfolder): 
    all_ids = get_ids(query_base, query_body, pages)
    #all_ids = load_ids()
...
main(wdir, a3o_base, query_base, query_body, pages, htmlfolder, txtfolder)
